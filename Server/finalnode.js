...
app.use(multer({ dest: '/home/cps/Desktop/Anuhya/'}).single('file'));
...
app.post('/sellerstatus', function(req,res){
		...
		MongoClient.connect(url, function(err, db) {
				...
				console.log(id);
				console.log("id");
				db.collection("mtt").find({seller:id}).count({},function(err,resl){
						if (err) throw err
						var count = resl;
						console.log(resl);
						if(resl==0)
						...
						db.collection("mtt").find({seller:id,msgid:"2"}).count({},function(err,r){
								if (err) throw err
								var count = r;
								console.log(r);
								if(r==0)
								...
								db.collection("mtt").find({seller:id,msgid:"2"}).toArray(function(err,resul){
										if (err) throw err;
										res.writeHead(200, { 'Content-Type': 'text/html' });
