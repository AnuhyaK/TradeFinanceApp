						...
						db.collection("mtt").find({buyerifsc:output.bankifsc}).count({},function(err,re){
							...
							db.collection("mtt").find({buyerifsc:output.bankifsc}).toArray(function(err,resul){
                                                                ...
                                                                else if(resul[i].msgid==2)
								{
                                                                ...
                                                                else if(resul[i].msgid==3 && resul[i].debited==1)
                                                                {
...
app.post('/damount', urlencodedParser,function(req,res){
		console.log(req.body);		
		var txid = parseInt(req.body.sid);
		res.cookie('txid',txid);
			MongoClient.connect(url, function(err, db) {
                                if (err) throw err;
     					db.collection("mtt").findOne({transid:txid},function(err,resul){
						if (err) throw err;
						if(resul.debited == 0)
						{
						console.log(resul);
							db.collection("User").findOne({userid:resul.buyer},function(err,result){
								if (err) throw err;
								console.log(result.balance);
								var newbalance =result.balance-resul.amount;
								if ( newbalance >= 0)
								{
								var update={$set:{balance:newbalance}};
									db.collection("User").update({userid:resul.buyer},update,function(err,result){
                                						if (err) throw err;
										var update ={$set:{debited:1}};
											db.collection("mtt").update({transid:txid},update,function(err,result){
												if (err) throw err;
												res.writeHead(200, { 'Content-Type': 'text/html' });
                                								res.write('<center><body bgcolor=Lightblue><head>Debited successfully! </head></body>')
												res.write('<br><form action="http://idrbtcps.com:8081/uploadpr" method ="POST"><button type="submit">upload payment receipt</button></form>');
												res.end();
											});
									});
								}
								...
								{
								res.writeHead(200, { 'Content-Type': 'text/html' });
								res.write('<center><body bgcolor=Lightblue><head>Insufficient Balance! </head></body>');
								res.write('<br><form action="http://idrbtcps.com:8081/buybank.html" method ="POST"> <button type="submit">Click here to GO Back</button></form>');
								res.end();
						...
						{
						res.writeHead(200, { 'Content-Type': 'text/html' });
                                		res.write('<center><body bgcolor=Lightblue><head>Already debited </head></body>');
						res.write('<br><form action="http://idrbtcps.com:8081/uploadpr" method ="POST"><button type="submit">upload payment receipt</button></form>');
						res.end();
						}
					});
...
app.post('/camount', urlencodedParser,function(req,res){
		console.log(req.body);		
		var txid = parseInt(req.body.sid);
		res.cookie('txid',txid);
			MongoClient.connect(url, function(err, db) {
     					...
     					db.collection("mtt").findOne({transid:txid},function(err,resul){
						if (err) throw err;
						if(resul.debited == 1 && resul.credited == 0)
						{
						console.log(resul);
							db.collection("User").findOne({username:resul.seller},function(err,result){
								if (err) throw err;
								console.log(result.balance);
								var newbalance =parseInt(result.balance) + parseInt(resul.amount);
								var update={$set:{balance:newbalance}};
									db.collection("User").update({username:resul.seller},update,function(err,result){
                                						if (err) throw err;
										var update ={$set:{credited:1, msgid:4}};
											db.collection("mtt").update({transid:txid},update,function(err,result){
												if (err) throw err;
												res.writeHead(200, { 'Content-Type': 'text/html' });
                                								res.write('<center><body bgcolor=Lightblue><head>credited successfully! </head></body>');
												res.write('<br><form action="http://idrbtcps.com:8081/buybank.html" method ="POST"><button type="submit">Log out</button></form>');
												res.end();
											});
									});
							});
						...
						{
						res.writeHead(200, { 'Content-Type': 'text/html' });
                                		res.write('<center><body bgcolor=Lightblue><head>Transaction failed! </head></body>');
						res.write('<br><form action="http://idrbtcps.com:8081/buybank.html" method ="POST"><button type="submit">Log out</button></form>');
						res.end();
						}
					});
...
app.post('/paymentstatus',urlencodedParser,function(req,res){
		...
		var out = __dirname +"/"+req.file.originalname;
			...
			var h = parseInt(req.cookies.txid);
                		...
                		MongoClient.connect(url, function(err, db) {
                                	if (err) throw err;
                                		db.collection("mtt").findOne({transid:h},function(err,result){
                                                	if (err) throw err;
							console.log(result);
							var update={ $set:{from:result.buyer,to:result.seller,paymentreceipt:req.file.originalname} };
                                                		db.collection("mtt").update({transid:h},update,function(err,result){
                                                                	res.writeHead(200, { 'Content-Type': 'text/html' });
					...
					else
					{
						db.collection("mtt").find({buyer:id}).toArray(function(err,resul){
							...
							if(resul[i].msgid == 1)
								...
								else if(resul[i].msgid==4 && ((resul[i].debited==1 && resul[i].credited==0)|| (resul[i].debited==0 && resul[i].credited==0)))
								{
								res.write('<br><center><h>Transaction ID:</h>&nbsp&nbsp'+resul[i].transid+'&nbsp&nbsp<h> STATUS :</h> &nbsp&nbsp '+resul[i].msgid+'&nbsp&nbsp<a href ="/'+resul[i].purchaseorder+'" >purchaseorder</a>&nbsp&nbsp<a href="/'+resul[i].lc+'">Letter of credit</a>&nbsp&nbsp<a href="/'+resul[i].shippingdoc+'">shipping Docs</a>');
								}
								else if(resul[i].msgid==4 && resul[i].debited == 1 && resul[i].credited == 1)
								{
...
app.post('/sstatus', function(req,res){
		MongoClient.connect(url, function(err, db) {
				if (err) throw err;
				var id = req.cookies.id;
				db.collection("User").find({userid:id}).toArray(function(err,result){
				console.log(result[0].username);				
				db.collection("mtt").find({seller:result[0].username}).count({},function(err,resul){
						if (err) throw err
						var count = resul;
						if(resul==0)
						{
						res.writeHead(200, { 'Content-Type': 'text/html' });
						res.write('<body bgcolor=Lightblue><center><h2> There are no transcations you are acting as  seller</h2></center></body>');
						res.write('<br><center><form action="http://idrbtcps.com:8081/b.html" method ="POST"><br> <button type="submit">click here to goto home page</button></form></center>');
						res.write('<br><center><form action="http://idrbtcps.com:8081/login.html" method ="POST"> <button type="submit">click here to  logout</button></form></center>');
						res.end();
						}
						...
						{
						db.collection("mtt").find({seller:result[0].username}).toArray(function(err,resul){
							if (err) throw err
								res.writeHead(200, { 'Content-Type': 'text/html' });
								res.write('<body bgcolor=Lightblue><center><head><h2> Status of transactions as seller</h2></head></center></body>');
								for(var i=0;i<count;i++)
								{
								if(resul[i].msgid == 1)
								{		
						...
						db.collection("mtt").find({buyerifsc:ifsc,msgid:"3"}).count({},function(err,re){
							...
							db.collection("mtt").find({buyerifsc:ifsc,msgid:"3"}).toArray(function(err,resul){
								...
								for(var i=0;i<count;i++)
                                                                ...
                                                                res.write('<br><form action = "http://idrbtcps.com:8081/damount" method = "POST"><br><input type="text" name="sid"><button type = "submit"> deduct amount</button> </form>');
							...
							db.collection("mtt").find({sellerifsc:ifsc}).toArray(function(err,resul){
								...
								for(var i=0;i<count;i++)
                                                                ...
                                                                res.write('<br><h>Transaction ID:</h>&nbsp&nbsp'+resul[i].transid+'&nbsp&nbsp<h> STATUS :</h> &nbsp&nbsp '+resul[i].msgid+'&nbsp&nbsp<a href ="/'+resul[i].purchaseorder+'" >purchaseorder</a>&nbsp&nbsp<a href="/'+resul[i].lc+'">Letter of credit</a>&nbsp&nbsp');
                                                                 ...
                                                                 else if(resul[i].msgid==3 && resul[i].credited==0)
                                                                {
                                                                res.write('<br><h>Transaction ID:</h>&nbsp&nbsp'+resul[i].transid+'&nbsp&nbsp<h> STATUS :</h> &nbsp&nbsp '+resul[i].msgid+'&nbsp&nbsp<a href ="/'+resul[i].purchaseorder+'" >purchaseorder</a>&nbsp&nbsp<a href="/'+resul[i].lc+'">Letter of credit</a>&nbsp&nbsp<a href="/'+resul[i].shippingdoc+'">shipping Docs</a>&nbsp&nbsp<a href="/'+resul[i].paymentreceipt+'">PaymentReceipt</a>');
								res.write('<br><form action="http://idrbtcps.com:8081/camount" method ="POST"><br>Enter a transaction Id to credit amount : <input type="text" name="sid"><br><button type="submit">Credit amount</button></form>');
								...
								else if(resul[i].msgid==4 && resul[i].credited==1)
                                                                {
                                                                res.write('<br><h>Transaction ID:</h>&nbsp&nbsp'+resul[i].transid+'&nbsp&nbsp<h> STATUS :</h> &nbsp&nbsp '+resul[i].msgid+'&nbsp&nbsp<a href ="/'+resul[i].purchaseorder+'" >purchaseorder</a>&nbsp&nbsp<a href="/'+resul[i].lc+'">Letter of credit</a>&nbsp&nbsp<a href="/'+resul[i].shippingdoc+'">shipping Docs</a>&nbsp&nbsp<a href="/'+resul[i].paymentreceipt+'">PaymentReceipt</a>');
					...
					db.collection("User").findOne({userid:req.cookies.id},function(err,resl){
						...
						console.log(resl);
							db.collection("mtt").find().count({},function(err,result){
								...
								//		console.log(resl);
								var inse = {transid:id,msgid:"1",from:resl.userid,to:resl.bankifsc,buyer:resl.userid,buyeraccno:resl.bankaccno,buyerbank:resl.bankname,buyerifsc:resl.bankifsc,seller:req.body.sname,sellerbank:req.body.sbank,selleracno:req.body.sacno,sellerifsc:req.body.sifsc,amount:req.body.amount,purchaseorder:req.file.originalname,lc:null,shippingdoc:null,lc:null,paymentreceipt:null,debited:"0",credited:"0"}; 
		...
		MongoClient.connect(url, function(err,db){
			...
			console.log(req.body.userid);
				db.collection("User").findOne({userid:req.body.userid},function(err,result){
					...
					if((result== null))
					{
					var myobj = { userid:req.body.userid,userpassword:req.body.password,username:req.body.username,bankname:req.body.bname,bankaccno:req.body.bacno,bankifsc:req.body.bifsc,balance:req.body.bal};
		...
		MongoClient.connect(url, function(err, db) {
			...
			console.log(id);
				db.collection("User").findOne({userid:id},function(err,result){
					if (err) throw err;
					res.writeHead(200, { 'Content-Type': 'text/html' });
                                	res.write('<center><body bgcolor=Lightblue><head><h3>User Details</h3></head></body>');
                                	res.write('<h>User Name:</h>&nbsp&nbsp'+result.username);
 					res.write('<br><h>User Id</h>:&nbsp&nbsp'+result.userid);
					res.write('<br><h>Bank Name</h>:&nbsp&nbsp'+result.bankname);
					res.write('<br><h>Bank Account No</h>:&nbsp&nbsp'+result.bankaccno);
					res.write('<br><h>Bank IFSC</h>:&nbsp&nbsp'+result.bankifsc);
					res.write('<br><form action="http://idrbtcps.com:8081/b.html" method ="POST"><br><button type="submit">Go Back</button></form>');
					res.end();
				});
		});
});
